...
**Bry's Edit:** A matrix and a data frame are both two-dimensional data structures. However, a matrix can only hold one type of data (e.g. numeric, character, logical) while a data frame can hold multiple types of data.
```{r}
...
```{r}
#Bry's Edit: To add column names to the hbirds data frame, we can use the colnames() function.

colnames(hbirds) <- c("Sex", "Length (in)", "Weight (oz)")
...
```{r}
#Bry's Edit: The dim() function returns the dimensions of the data frame. The str() function returns the structure of the data frame.
